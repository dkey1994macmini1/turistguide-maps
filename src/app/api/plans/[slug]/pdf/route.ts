...
export async function GET(
  ...
  for (const day of plan.days) {
    // Day header
    doc.fontSize(14).fillColor("#2d6a4f").text(`Dzień ${day.dayNumber}: ${day.title || ""}`);
    if (day.description) {
    ...
    for (const stop of day.stops) {
      doc.fontSize(11).fillColor("#333").text(`• ${stop.title}`, { continued: false });
      ...
      if (stop.bring && stop.bring.length > 0) {
        doc.fontSize(8).fillColor("#888").text(`  🎒 ${stop.bring.join(", ")}`);
      }
      // Google Maps link
      if (stop.googleMapsUrl) {
        doc.fontSize(8).fillColor("#2d6a4f").text(`  📍 ${stop.googleMapsUrl}`, { link: stop.googleMapsUrl });
      }
      doc.moveDown(0.3);
    }
    doc.moveDown(0.5);
